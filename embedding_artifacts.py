...
from embedding_support import write_json
...
def _bootstrap_mean_ci(values: np.ndarray, seed: int = 42, repeats: int = 2_000) -> tuple[float, float, float]:
    if values.size == 0:
    ...
    *,
    ...
    for batch in loader:
        ...
        if labels.shape != predicted.shape[:2]:
            raise ValueError(f"Label shape {tuple(labels.shape)} does not match prediction shape {tuple(predicted.shape)}")
        ...
        predicted_labels_out.append(predicted_labels.detach().cpu().numpy().astype(np.int64, copy=False))
        ...
        for local_index in range(batch_size):
            ...
            expected_actions = [action_pool[int(label)] for label in labels[local_index].detach().cpu().tolist()]
        ...
        padded_out.append(
            np.repeat(
                np.asarray([bool(dataset.metadata_at(offset + index)["is_padded"]) for index in range(batch_size)])[:, None],
                horizon,
                axis=1,
            )
        )
...
def summarize_rows(rows: list[dict[str, Any]], split_name: str, arrays: Mapping[str, np.ndarray]) -> dict[str, Any]:
    summary_rows: list[dict[str, Any]] = []
    for step in sorted({int(row["step_index"]) for row in rows}):
        step_rows = [row for row in rows if int(row["step_index"]) == step]
        record: dict[str, Any] = {"split": split_name, "step_index": step, "samples": len(step_rows)}
    ...
    overall = {"split": split_name, "samples": int(arrays["mse"].shape[0]), "steps": int(arrays["mse"].shape[1])}
...
def write_combined_summary(output_dir: Union[str, Path], exports: Mapping[str, Mapping[str, Any]]) -> None:
    root = Path(output_dir)
    ...
    with (root / "summary_metrics.csv").open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writeheader()
        writer.writerows(rows)
    write_json(root / "summary_metrics.json", {name: exported["summary"] for name, exported in exports.items()})
...
def _plot_mean_ci(axis: Any, rows: list[dict[str, Any]], metric: str, label: str, color: str) -> None:
    ...
    for step in steps:
        ...
        lows.append(low)
        highs.append(high)
    axis.errorbar(steps, means, yerr=[np.asarray(means) - np.asarray(lows), np.asarray(highs) - np.asarray(means)],
                  marker="o", capsize=3, label=label, color=color)


def render_figures(output_dir: Union[str, Path], exports: Mapping[str, Mapping[str, Any]]) -> None:
    ...
    for metric, filename, ylabel, zero_line in (
        ...
        ("candidate_margin", "candidate_margin_by_step.png", "GT cosine − strongest non-GT cosine", 0.0),
    ):
        figure, axis = plt.subplots(figsize=(7, 4.5))
        _plot_mean_ci(axis, base_rows, metric, "Base", "#1f77b4")
        _plot_mean_ci(axis, novel_rows, metric, "Novel", "#ff7f0e")
        if metric == "mse":
            for name, exported, color in (("Base zero-vector", exports["base"], "#1f77b4"), ("Novel zero-vector", exports["novel"], "#ff7f0e")):
                axis.axhline(exported["summary"]["overall"]["zero_vector_mse"], linestyle="--", color=color, alpha=0.6, label=name)
    ...
    for split_name, rows, color in (("Base", base_rows, "#1f77b4"), ("Novel", novel_rows, "#ff7f0e")):
        for step in sorted({int(row["step_index"]) for row in rows}):
            labels.append(f"{split_name}\nstep {step + 1}")
            distributions.append([float(row["cosine"]) for row in rows if int(row["step_index"]) == step])
            colors.append(color)
    ...
    for axis, metric, title in zip(axes, ("cosine", "candidate_margin"), ("Cosine", "Candidate margin")):
        distributions, labels = [], []
        for name, rows in (("correct", [row for row in base_rows + novel_rows if int(row["correct"]) == 1]), ("wrong", [row for row in base_rows + novel_rows if int(row["correct"]) == 0])):
            if rows:
    ...
    for split_name, rows, color in (("Base", base_rows, "#1f77b4"), ("Novel", novel_rows, "#ff7f0e")):
        for category, subset in (("all", rows), ("non-padded", [row for row in rows if not bool(row["is_padded"])])):
            labels.append(f"{split_name}\n{category}")
            values.append(float(np.mean([float(row["cosine"]) for row in subset])) if subset else float("nan"))
            colors.append(color)
...
def save_candidate_embeddings(output_dir: Union[str, Path], embeddings: Mapping[str, torch.Tensor], pools: Mapping[str, list[str]]) -> None:
    root = Path(output_dir) / "raw"
    root.mkdir(parents=True, exist_ok=True)
    payload: dict[str, Any] = {}
    for split_name, tensor in embeddings.items():
        payload[f"{split_name}_embeddings"] = tensor.detach().cpu().numpy().astype(np.float32, copy=False)
