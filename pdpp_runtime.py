...
from embedding_support import utc_now
...
def checkpoint_payload(
    *,
    ...
    return {
        ...
        "created_at": utc_now(),
        "epoch": int(epoch),
        "model": {key: value.detach().cpu() for key, value in unwrap_model(model).state_dict().items()},
        "ema": {key: value.detach().cpu() for key, value in unwrap_model(ema_model).state_dict().items()},
...
def save_pdpp_checkpoint(path: Union[str, Path], payload: Mapping[str, Any]) -> None:
...
def load_pdpp_checkpoint(path: Union[str, Path], device: torch.device) -> dict[str, Any]:
    try:
