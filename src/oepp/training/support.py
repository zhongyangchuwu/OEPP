...
def load_action_embedding_dict(data_root: str | Path, feature: str) -> dict[str, Any]:
    filename = {
...
def load_action_pool(data_root: str | Path, split: int, partition: str) -> list[str]:
    filename = {
...
def action_embedding_tensor(
    ...
    try:
        vectors = [
            torch.as_tensor(action_embedding_dict[action], dtype=torch.float32)
...
def load_direct_checkpoint(
    path: str | Path, device: torch.device, *, trust_checkpoint: bool = False
) -> dict[str, Any]:
    if trust_checkpoint:
...
def annotation_hashes(
    data_root: str | Path, split: int, feature: str = "videoclip"
) -> dict[str, str]:
    root = Path(data_root)
