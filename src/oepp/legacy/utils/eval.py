...
def validate(val_loader1, val_loader2, val_loader3, val_loader4, model, args, text_tensor):
    ...
    for i_batch, sample_batch in enumerate(val_loader1):
        with torch.no_grad():
            ...
            labels = labels.cuda()
            # T1 = 2
            cond = {0: start.cuda().contiguous().float(), T - 1: end.cuda().contiguous().float()}
            ...
            for j in range(T):
                ...
                sim_logits = torch.nn.functional.cosine_similarity(
                    frames_embedding.unsqueeze(1), text_tensor.unsqueeze(0), dim=2
                )  # 256 666
            ...
            (acc1, acc5), trajectory_success_rate, MIoU1, MIoU2, a0_acc, aT_acc = accuracy(
                pred_logits.cpu(), labels.view(-1).cpu(), topk=(1, 5), max_traj_len=T
            )
    ...
    return (
        torch.tensor(acc_top11.avg),
        torch.tensor(acc_top51.avg),
        torch.tensor(trajectory_success_rate_meter1.avg),
        torch.tensor(MIoU1_meter1.avg),
        torch.tensor(MIoU2_meter1.avg),
        torch.tensor(A0_acc1.avg),
        torch.tensor(AT_acc1.avg),
    )
