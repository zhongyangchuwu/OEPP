...
class Conv1dBlock(nn.Module):
    ...
    def __init__(
        self, inp_channels, out_channels, kernel_size, n_groups=32, drop_out=0.0, if_zero=False
    ):
        super().__init__()
        if drop_out > 0.0:
...
def condition_projection(x, conditions, action_dim, class_dim, horizon_dim):
    ...
    x[:, 1:-1, horizon_dim + class_dim + action_dim :] = 0.0
...
def get_lr_schedule_with_warmup(optimizer, num_training_steps, last_epoch=-1):
    ...
    def lr_lambda(current_step):
        if current_step <= num_warmup_steps:
            ...
            return max(0.5 ** ((current_step - num_warmup_steps) // decay_steps), 0.0)
...
class Logger:
    ...
    def log_scalars(self, scalar_dict, group_name, step, phase):
        ...
        self._summ_writer.add_scalars(f"{group_name}_{phase}", scalar_dict, step)
