...
class TransformerEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, num_heads, T, max_seq_len=10):
        ...
        self.state_linears = nn.ModuleList([nn.Linear(2 * input_dim, hidden_dim) for _ in range(T)])
        ...
        self.positional_encoding = nn.Parameter(
            self.generate_positional_encoding(max_seq_len, hidden_dim), requires_grad=False
        )
    ...
    def forward(self, inputs):
        ...
        x_2 = x
        x = torch.stack([linear(x_2) for linear in self.state_linears], dim=1)
        # B*T*D
