...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        # Clamp before log because posterior variance is zero at the first diffusion step.
    ...
    def p_sample_ddim(self, x, cond, t, t_prev, if_prev=False, generator=None):
        ...
        mean_pred = (
            x_recon * torch.sqrt(alpha_bar_prev) + torch.sqrt(1 - alpha_bar_prev - sigma**2) * eps
        )
    ...
    def p_sample_loop(self, cond, length, if_jump, generator=None):
        ...
        shape = (
            batch_size,
            horizon,
            self.class_dim + self.action_dim + self.observation_dim + self.horizon_dim,
        )
            ...
            for i in reversed(range(0, self.ddim_timesteps)):
                ...
                if i == 0:
                    ...
                    x = self.p_sample_ddim(
                        x, cond, timesteps, timesteps_prev, True, generator=generator
                    )
                else:
                    timesteps_prev = torch.full(
                        (batch_size,),
                        self.ddim_timestep_seq[i - 1],
                        device=device,
                        dtype=torch.long,
                    )
    ...
    def p_losses(self, x_start, cond, t):
        ...
        x_noisy = condition_projection(
            x_noisy, cond, self.action_dim, self.class_dim, self.horizon_dim
        )

        x_recon = self.model(x_noisy, t, cond)
        x_recon = condition_projection(
            x_recon, cond, self.action_dim, self.class_dim, self.horizon_dim
        )
        return x_recon
