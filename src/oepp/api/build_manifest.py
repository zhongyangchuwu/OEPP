...
from .common import REPOSITORY_ROOT, load_jsonl, write_json, write_jsonl
...
def build_manifest(arguments: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    partition_by_split = {"base": Partition.BASE_TEST, "novel": Partition.NOVEL_TEST}
    pool_by_split = {"base": ActionPool.BASE, "novel": ActionPool.NOVEL}
    for split in arguments.split:
        records = bundle.partition_records(partition_by_split[split])
        selected_pool = (
            bundle.action_pool(ActionPool.TOTAL)
            if arguments.pool_type == "total"
            else bundle.action_pool(pool_by_split[split])
        )
                        ...
                        validation_errors.append(
                            {
                                ...
                                "image_setting": image_setting,
                                "error": (
                                    f"ground truth missing from candidate pool: {missing_actions}"
                                ),
                            }
                        )
                        continue
